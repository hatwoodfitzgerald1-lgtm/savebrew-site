...
const VITE_STUB = `const noop = () => {}; export const createHotContext = () => ({ accept: noop, acceptExports: noop, dispose: noop, prune: noop, decline: noop, invalidate: noop, on: noop, off: noop, send: noop, data: {} }); export const injectQuery = (u) => u; export const updateStyle = () => {}; export const removeStyle = () => {}; export default {};`;
...
for (const width of widths) {
  ...
  await page.route('**/@vite/client', (r) => r.fulfill({ body: VITE_STUB, contentType: 'application/javascript' }));
  ...
  await page.evaluate(async () => {
    ...
    for (let y = 0; y < h; y += Math.round(innerHeight * 0.6)) { scrollTo({ top: y, behavior: 'instant' }); await new Promise((r) => setTimeout(r, 90)); }
  ...
  if (!flags.has('--fold')) {
    ...
    await page.evaluate(async () => { scrollTo({ top: document.documentElement.scrollHeight, behavior: 'instant' }); await new Promise((r) => setTimeout(r, 600)); const b = document.querySelector('.band'); if (b) { b.style.position = 'relative'; b.classList.remove('is-hidden'); } const t = document.querySelector('.tabstrip'); if (t) t.style.position = 'relative'; });
